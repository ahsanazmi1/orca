...
class TestTraceLoggerAdapter:
    ...
    def test_auto_trace_id_from_context(self):
        ...
        with patch("src.orca.logging_setup.get_current_trace_id", return_value="auto-trace-456"):
            # Capture log output
            handler = logging.StreamHandler(StringIO())
            handler.setLevel(logging.INFO)
            logger.addHandler(handler)
...
class TestGetLogger:
    ...
    def test_get_traced_logger(self):
        ...
        with patch("src.orca.logging_setup.get_current_trace_id", return_value="current-trace-999"):
...
class TestLoggingIntegration:
    ...
    def test_setup_logging_with_json_format(self):
        ...
        from src.orca.logging_setup import JSONFormatter, RedactionFilter
    ...
    def test_redaction_with_trace_binding(self):
        ...
        with patch(
            "src.orca.logging_setup.get_current_trace_id", return_value="redaction-trace-222"
        ):
