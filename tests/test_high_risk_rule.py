...
from orca_core.rules import HighRiskRule
...
class TestHighRiskRule:
    ...
    def test_high_risk_rule_triggered(self) -> None:
        ...
        with patch("orca_core.rules.high_risk.predict_risk", return_value=0.95):
    ...
    def test_high_risk_rule_not_triggered(self) -> None:
        ...
        with patch("orca_core.rules.high_risk.predict_risk", return_value=0.50):
            rule = HighRiskRule(threshold=0.80)
            request = DecisionRequest(
                cart_total=100.0, features={"velocity_24h": 2.0, "customer_age": 30}
            )
    ...
    def test_high_risk_rule_custom_threshold(self) -> None:
        ...
        with patch("orca_core.rules.high_risk.predict_risk", return_value=0.70):
    ...
    def test_high_risk_rule_calls_predict_risk(self) -> None:
        ...
        with patch("orca_core.rules.high_risk.predict_risk", return_value=0.15) as mock_predict:
            rule = HighRiskRule(threshold=0.80)
