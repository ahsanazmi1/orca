            ...
            if has_approve and has_decline and has_review:
                self.add_result(
                    AuditResult(
                        ...
                        status="pass",
                        evidence=[
                            f"Found {len(ap2_samples)} AP2 samples with approve/decline/review examples"
                        ],
                    )
                )
            else:
        ...
        if rules_files:
            ...
            for rules_file in rules_files:
                ...
                for _, _, line in matches:
                    if '"APPROVE"' in line or "'APPROVE'" in line:
                        has_approve = True
                    if '"DECLINE"' in line or "'DECLINE'" in line:
                        has_decline = True
            ...
            if len(found_keys) >= 3:  # At least 3 of 4 keys
                ...
                self.add_result(
                    AuditResult(
                        ...
                        status="warning",
                        evidence=[
                            f"Found keys: {', '.join(found_keys)}, missing: {', '.join(set(required_keys) - set(found_keys))}"
                        ],
        ...
        if git_tags:
            self.add_result(
                AuditResult(
                    ...
                    status="pass",
                    evidence=[
                        f"Found {len(git_tags)} git tags: {', '.join(git_tags[:5])}"
                        + ("..." if len(git_tags) > 5 else "")
                    ],
                )
            )
        else:
    ...
    def generate_json_report(self) -> None:
        ...
        report_data = {
            ...
            "summary": {
                "foundations": {
                    "passed": len(
                        [r for r in self.results if r.phase == "Foundations" and r.status == "pass"]
                    ),
                    "total": len([r for r in self.results if r.phase == "Foundations"]),
                },
                "phase1": {
                    "passed": len(
                        [r for r in self.results if r.phase == "Phase 1" and r.status == "pass"]
                    ),
                    "total": len([r for r in self.results if r.phase == "Phase 1"]),
                },
                "phase2": {
                    "passed": len(
                        [r for r in self.results if r.phase == "Phase 2" and r.status == "pass"]
                    ),
